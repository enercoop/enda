...
import enda
from enda.estimators import EndaStackingEstimator, EndaEstimatorWithFallback, EndaEstimator
...
class LoadForecast(EndaEstimator):
    ...
    def predict(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        prediction[LoadForecast.TARGET] = prediction[LoadForecast.TARGET].round(decimals=3)
        return prediction
    ...
    def expected_columns_in_order(cls, train_or_test: str):
        ...
        columns = [
            ...
            "dayofyear_cos", "dayofyear_sin"  # use dayofyear for the yearly "cyclic" feature
        ]
        ...
        if train_or_test == 'train':
            ...
            raise ValueError("unexpected argument train_or_test: {}".format(train_or_test))
    ...
    def check_df(cls, train_or_test: str, df: pd.DataFrame):
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            raise ValueError("index must be a pandas.DatetimeIndex, but found '{}'".format(df.index.dtype))
        if df.index.name != LoadForecast.INDEX_NAME:
            raise ValueError("index name must be '{}', but found '{}'"
                             .format(LoadForecast.INDEX_NAME, df.index.name))
        if str(df.index.tz) != LoadForecast.TZINFO_STR:
            raise ValueError("index must have timezone '{}', but found '{}'"
                             .format(LoadForecast.TZINFO_STR, df.index.tz))
        ...
        if list(df.columns) != expected_columns:
            raise ValueError("Expected columns: {}, but given df with columns: {}"
                             .format(expected_columns, df.columns))
        ...
        if count_na > 0:
            ...
            raise ValueError("Trying to predict on a dataframe with {} NaN values\n{}"
                             .format(count_na, df_nan))
    ...
    def __get_untrained_base_estimators(cls):
        ...
        nn = EndaH2OEstimator(H2ODeepLearningEstimator(
            **{
...
class PortfolioData:
    ...
    def get_forecast_portfolio(cls, portfolio, start_forecast_date, nb_days):
        ...
        min_time = TimezoneUtils.add_interval_to_day_dt(day_dt=start_forecast_date, interval=relativedelta(days=-90))
        max_time = start_forecast_date  # don't use future events
        pf_last_90days = portfolio[(min_time <= portfolio.index) & (portfolio.index < max_time)]
...
class LoadForecastData:
    ...
    logger_handler.setFormatter(
        logging.Formatter(fmt='[%(levelname)s %(asctime)s %(filename)s:%(lineno)s] %(message)s',
    ...
    def log_dataframe_info(cls, df, title, logger):
        ...
        freq, missing_periods, extra_points = enda.TimeSeries.find_missing_and_extra_periods(
            dti=df.index,
            expected_freq=LoadForecast.FREQ_STR
        )
        logger.info("{}:\n"
                    "Shape: {}\n"
                    "Columns: {}\n"
                    ...
                    "Missing periods:\n{}\n"
                    "Extra points:\n{}"
                    .format(title, df.shape, df.columns, freq, df.index.min(), df.index.max(),
                            missing_periods, extra_points))
    ...
    def compute_historic_df(cls, logger):
        ...
        cls.log_dataframe_info(historic, title="historic dataframe to train short-term consumption forecast",
                               logger=logger)
    ...
    def compute_forecast_input_df(cls, logger, run_date: [pd.Timestamp, None] = None):
        ...
        if run_date.tzinfo is None or str(run_date.tzinfo) != LoadForecast.TZINFO_STR:
            raise ValueError("run_date must be at timezone {} but given '{}'"
                             .format(LoadForecast.TZINFO_STR, run_date.tzinfo))
        ...
        forecast_portfolio = PortfolioData.get_forecast_portfolio(
            portfolio=historic_portfolio,
            start_forecast_date=tomorrow,
            nb_days=11
        )

        special_days = Calendar().get_french_special_days()
        forecast_tso = TsoData.get_future_tso_forecast(start_forecast_date=tomorrow, nb_days=7)
        forecast_weather = WeatherData.get_future_weather_forecast(start_forecast_date=tomorrow, nb_days=11)
        ...
        logger.info("Profiled future dataframe components (run_date = {}) :".format(run_date))
        logger.info("--- Portfolio : start = {}, end = {}"
                    .format(forecast_portfolio.index.min(), forecast_portfolio.index.max()))
        logger.info("--- Weather   : start = {}, end = {}"
                    .format(forecast_weather.index.min(), forecast_weather.index.max()))
        logger.info("--- Calendar  : start = {}, end = {}"
                    .format(special_days.index.min(), special_days.index.max()))
        logger.info("--- RTE       : start = {}, end = {}"
                    .format(forecast_tso.index.min(), forecast_tso.index.max()))
        logger.info("--- Result    : start = {}, end = {}"
                    .format(df.index.min(), df.index.max()))

        df = cls.featurize_datetime_index(df)

        cls.log_dataframe_info(df, title="Input dataframe to predict short-term consumption", logger=logger)
    ...
    def read_local_dataframe(file_path):
        ...
        df[LoadForecast.INDEX_NAME] = enda.TimeSeries.align_timezone(df[LoadForecast.INDEX_NAME],
                                                                     tzinfo=LoadForecast.TZINFO_STR)
        df.set_index(LoadForecast.INDEX_NAME, drop=True, inplace=True)
        return df

    @classmethod
    def get_df(cls, dataset, limit_train_data, read_local_dataset, save_dataset_locally, logger):
        ...
        if dataset == 'historic':
            ...
            raise ValueError("Unexpected dataset '{}'. Expecting 'historic' or 'forecast_input'.".format(dataset))
    ...
    @classmethod
    def get_historic_df(cls, limit_train_data=0, read_local_dataset=False, save_dataset_locally=True):
        return cls.get_df('historic', limit_train_data, read_local_dataset, save_dataset_locally)

    @classmethod
    def get_forecast_input_df(cls, limit_train_data=0, read_local_dataset=False, save_dataset_locally=True):
        return cls.get_df('forecast_input', limit_train_data, read_local_dataset, save_dataset_locally)
...
def main():
    ...
    parser.add_argument("--task", action="store", required=True, choices=["learn", "predict"],
    ...
    parser.add_argument("--read_local_data", action="store_true", default=False,
    ...
    parser.add_argument("--learn_data_limit", action="store", type=int, default=0,
    ...
    parser.add_argument("--send_prediction_to_cofycloud", action="store_true", default=False,
    ...
    if args.task == 'learn':
        ...
        with open(LoadForecastData.MODEL_FILE_PATH, "wb") as f:
            pickle.dump(algo, f)
        logger.info("Saved trained algo at : {}".format(LoadForecastData.MODEL_FILE_PATH))
    ...
    if args.task == 'predict':
        ...
        forecast_input = LoadForecastData.get_forecast_input_df(read_local_dataset=args.read_local_data)
