...
class TestTimeSeries(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.almost_imperfect_dti = pd.DatetimeIndex(
            [
                ...
                "2024-01-01 02:00:00+01",
                "2024-01-01 02:00:00+01",
                "2024-01-01 04:00:00+01",
                "2024-01-01 03:00:00+01",
                "2024-01-01 05:00:00+01",
            ]
        )  # 1H duplicates unordered
        self.imperfect_dti = pd.DatetimeIndex(
            [
                "2024-01-01 01:00:00+01",
                "2024-01-01 02:00:00+01",
                "2024-01-01 03:00:00+01",
                "2024-01-01 06:00:00+01",
                "2024-01-01 08:00:00+01",
                "2024-01-01 09:00:00+01",
                "2024-01-01 09:30:00+01",
            ]
        )  # 1H with holes and an extra period (last point)
        self.monthly_dti = pd.DatetimeIndex(
    ...
    def test_split_amount_and_unit_from_freq(self):
        ...
        parsed_freq_list = [
            TimeSeries.split_amount_and_unit_from_freq(_) for _ in self.ok_freq_list
        ]
    ...
    def test_freq_as_approximate_nb_days(self):
        ...
        # test with correct frequencies
        result_list = [
            TimeSeries.freq_as_approximate_nb_days(_) for _ in self.ok_freq_list
        ]
    ...
    def test_add_timedelta(self):
        ...
        result_list = [
            ...
            for freq in self.ok_freq_list[:-1]
        ]

        expected_result_list = [
            pd.to_datetime("2021-01-03 00:00:00"),
        ...
        result_list = [
            TimeSeries.add_timedelta(test_timestamp_tz, freq)
            for freq in self.ok_freq_list[:-1]
        ]

        expected_result_list = [
            pd.Timestamp("2021-01-03 00:00:00+01"),
        ...
        result_list = [
            TimeSeries.add_timedelta(test_date, freq) for freq in self.ok_freq_list[:-1]
        ]
        ...
        expected_result_list = [
            datetime.datetime(year=2021, month=1, day=3, hour=2),
    ...
    def test_subtract_timedelta(self):
        ...
        test_timestamp = pd.to_datetime("2021-01-01 00:00:00")
        result_list = [
            TimeSeries.subtract_timedelta(test_timestamp, freq)
            for freq in self.ok_freq_list[:-1]
        ]

        expected_result_list = [
            pd.to_datetime("2020-12-30 00:00:00"),
    ...
    def test_find_gap_distribution(self):
        ...
        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(self.dti),
            pd.Series(data=[3], index=[pd.Timedelta("1H")]),
        )

        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(self.dti, True),
            pd.Series(data=[3], index=[pd.Timedelta("1H")]),
        )

        # test almost_imperfect_dti
        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(self.almost_imperfect_dti),
            pd.Series(data=[4, 1], index=[pd.Timedelta("1H"), pd.Timedelta("0H")]),
        )

        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(
                dti=self.almost_imperfect_dti, skip_duplicate_timestamps=True
            ),
            pd.Series(data=[4], index=[pd.Timedelta("1H")]),
        )

        # test dti_imperfect
        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(self.imperfect_dti),
            pd.Series(
                data=[3, 1, 1, 1],
                index=[
                    pd.Timedelta("1H"),
        ...
        # test monthly data
        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(self.monthly_dti),
            pd.Series(data=[1, 1], index=[pd.Timedelta("31D"), pd.Timedelta("29D")]),
        )

        # test imperfect monthly data
        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(dti=self.almost_imperfect_monthly_dti),
            pd.Series(
                data=[1, 1, 1],
        ...
        # test series
        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(self.series),
            pd.Series(data=[3], index=[pd.Timedelta("1H")]),
        )

        pd.testing.assert_series_equal(
            TimeSeries.find_gap_distribution(
                self.series, skip_duplicate_timestamps=True
            ),
            pd.Series(data=[3], index=[pd.Timedelta("1H")]),
        )
    ...
    def test_find_most_common_frequency(self):
        ...
        # test almost_imperfect_dti
        self.assertEqual(
            TimeSeries.find_most_common_frequency(self.almost_imperfect_dti), "1H"
        )
        self.assertEqual(
            TimeSeries.find_most_common_frequency(
                dti=self.almost_imperfect_dti, skip_duplicate_timestamps=True
            ),
            "1H",
        )
        ...
        # test dti_imperfect
        self.assertEqual(
            TimeSeries.find_most_common_frequency(self.imperfect_dti), "1H"
        )
        ...
        self.assertEqual(
            TimeSeries.find_most_common_frequency(self.almost_imperfect_monthly_dti),
            "31D",
        )
        self.assertEqual(
            TimeSeries.find_most_common_frequency(
                self.almost_imperfect_monthly_dti, skip_duplicate_timestamps=True
            ),
            "1MS",
        )
    ...
    def test_find_duplicates(self):
        ...
        pd.testing.assert_series_equal(
            TimeSeries.find_duplicates(self.series), empty_series
        )

        pd.testing.assert_index_equal(
            TimeSeries.find_duplicates(self.almost_imperfect_dti),
            pd.DatetimeIndex(["2024-01-01 02:00:00+01"]),
        )
        pd.testing.assert_index_equal(
            TimeSeries.find_duplicates(self.almost_imperfect_monthly_dti),
    ...
    def test_find_extra_points(self):
        ...
        pd.testing.assert_index_equal(
            TimeSeries.find_extra_points(self.dti), result_index
        )

        result_series = pd.Series(data=[], dtype="datetime64[ns]")
        pd.testing.assert_series_equal(
            TimeSeries.find_extra_points(self.series), result_series
        )

        # extra periods are found from the start
        result_index = pd.DatetimeIndex(["2024-01-01 02:00:00", "2024-01-01 04:00:00"])
        pd.testing.assert_index_equal(
            TimeSeries.find_extra_points(self.dti, expected_freq="2H"), result_index
        )

        result_series = pd.Series(
            data=["2024-01-01 02:00:00", "2024-01-01 04:00:00"], dtype="datetime64[ns]"
        )
        pd.testing.assert_series_equal(
            TimeSeries.find_extra_points(self.series, expected_freq="2H"), result_series
        )
        ...
        self.assertEqual(
            len(TimeSeries.find_extra_points(self.almost_imperfect_dti)), 0
        )

        result_index = pd.DatetimeIndex(["2024-01-01 09:30:00+01"])
        pd.testing.assert_index_equal(
            TimeSeries.find_extra_points(self.imperfect_dti), result_index
        )

        self.assertEqual(len(TimeSeries.find_extra_points(self.monthly_dti)), 0)
        self.assertEqual(
            len(TimeSeries.find_extra_points(self.almost_imperfect_monthly_dti)), 0
        )
    ...
    def test_find_missing_points(self):
        ...
        pd.testing.assert_index_equal(
            ...
            pd.DatetimeIndex(
                ["2024-01-01 01:55:00", "2024-01-01 02:50:00", "2024-01-01 03:45:00"]
            ),
        )
    ...
    def test_has_single_frequency(self):
        ...
        self.assertEqual(
            TimeSeries.has_single_frequency(
                self.imperfect_dti,
        ...
        self.assertEqual(
            TimeSeries.has_single_frequency(
                self.imperfect_dti, skip_duplicate_timestamps=True
            ),
            False,
        )
        ...
        self.assertEqual(
            TimeSeries.has_single_frequency(
                self.monthly_dti, variable_duration_freq_included=False
            ),
            False,
        )
    ...
    def test_find_missing_and_extra_periods(self):
        ...
        self.assertEqual(
            pd.Timedelta(freq), pd.Timedelta("15min")
        )  # inferred a 15min freq
        self.assertEqual(
            len(missing_periods), 2
        )  # (01:15:00 -> 01:45:00), (02:15:00 -> 02:15:00)
        self.assertEqual(len(extra_points), 2)  # [00:50:00, 02:20:00]
    ...
    def test_interpolate_daily_to_sub_daily_data(self):
        ...
        expected_df1["value"] = [0.0] * 8 + [1.0] * 8
        ...
        expected_df2["value"] = [2.0] * 8 + [3.0] * 4
        expected_df2["key"] = "key2"
        expected_df2.index.freq = "6H"

        expected_df = pd.concat([expected_df1, expected_df2], axis=0).set_index(
            ["key", "time"]
        )
    ...
    def test_interpolate_freq_to_sub_freq_data(self):
        ...
        expected_df.index.freq = "6H"
        expected_df["value"] = [0.0] * 5 + [0.25, 0.5, 0.75] + [1] * 5
        ...
        expected_df1["value"] = [0.0] * 8 + [1.0] * 5
        expected_df1["key"] = "key1"
        expected_df1.index.freq = "6H"
        ...
        expected_df2["value"] = [2.0] * 8 + [3.0] * 1
        expected_df2["key"] = "key2"
        expected_df2.index.freq = "6H"
    ...
    def test_forward_fill_final_record(self):
        ...
        expected_df["value"] = [0.0, 1.0, 2.0, 2.0]
        expected_df.index.freq = "12H"

        sub_df = TimeSeries.forward_fill_final_record(df=test_df, gap_frequency="1D")
        pd.testing.assert_frame_equal(expected_df, sub_df)
        ...
        expected_df["value"] = [0.0, 1.0, 2.0, 3.0, 3.0, 3.0]
        expected_df.index.freq = "1H"

        sub_df = TimeSeries.forward_fill_final_record(
            df=test_df, gap_frequency="3H", cut_off_frequency=None
        )
        pd.testing.assert_frame_equal(expected_df, sub_df)
        ...
        expected_df["value"] = [0.0, 1.0, 2.0, 3.0, 3.0]
        expected_df.index.freq = "1H"

        sub_df = TimeSeries.forward_fill_final_record(
            df=test_df, gap_frequency="3H", cut_off_frequency="1D"
        )
        pd.testing.assert_frame_equal(expected_df, sub_df)
